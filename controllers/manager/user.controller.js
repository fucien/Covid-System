...
user.post("/create", async (req, res) => {
  ...
  try {
    ...
    if (req.body.ngaysinh) {
      DOB = req.body.ngaysinh;
      DOBReverse = req.body.ngaysinh.split("-").reverse().join("-");
    }
    ...
    if (result) {
      accountInfo.NguoiLienQuan = result.MaNguoiLienQuan;
      const accountInserted = await userModel.createAccount(accountInfo);
      if (accountInserted) {
        res.cookie("createUser", "Thêm người liên quan covid thành công.");
      } else {
        res.cookie(
          "createUser",
          "Thêm người liên quan covid không thành công."
        );
      }
    } else {
